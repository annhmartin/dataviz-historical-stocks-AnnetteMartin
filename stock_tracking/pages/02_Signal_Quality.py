...
if daily_signals.empty: st.error("No signal data."); st.stop()
...
if price.empty or sig.empty: st.warning("No data for selected ticker."); st.stop()
...
for _, srow in sig.iterrows():
    ...
    ep = price.loc[future[0], "Close"]; xp = price.loc[future[hold_days-1], "Close"]
    ret = (xp - ep) / ep * 100
...
if not rows: st.info("Not enough signal days for this ticker and date range."); st.stop()

df_plot = pd.DataFrame(rows)
counts = df_plot["q"].value_counts(); total = len(df_plot)
...
ax.scatter(df_plot["sent"], df_plot["ret"], c=df_plot["c"], alpha=0.65, s=df_plot["stories"].clip(1,50)*8+15, edgecolors="white", linewidths=0.4, zorder=3)
ax.axhline(0, color="#888888", linewidth=1); ax.axvline(0, color="#888888", linewidth=1)
xmax = float(df_plot["sent"].abs().max())*1.1; ymax = float(df_plot["ret"].abs().max())*1.1
...
if len(df_plot) > 5:
    z = np.polyfit(df_plot["sent"], df_plot["ret"], 1); xs = np.linspace(-xmax,xmax,100)
