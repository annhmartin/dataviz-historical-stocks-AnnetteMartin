...
from utils import load_csv, load_signals, sidebar_filters, get_sig_col, CORR_PREFIX, SENTIMENT_THRESHOLD, apply_chart_style, POS, NEG, NEU, INK, AXIS, MUTED, GRID, CANVAS, POS_FILL, NEG_FILL, MARKER_SIGNAL
...
fig, ax = plt.subplots(figsize=(11, fig_height), facecolor=CANVAS, dpi=100)
colors = [NEG if v < 0 else POS for v in plot_df["corr"]]
...
if daily_signals.empty:
    ...
    if not valid:
        ...
        if sig[sig_col].notna().sum() > 20:
            ...
            fig2, ax2 = plt.subplots(figsize=(14, 4), facecolor=CANVAS, dpi=100)
            ax2.plot(sig["date"], sig["roll_acc"], color=MARKER_SIGNAL, linewidth=2)
            ax2.axhline(0.5, color=AXIS, linewidth=1, linestyle="--")
            ax2.fill_between(sig["date"], 0.5, sig["roll_acc"],
                             where=sig["roll_acc"] >= 0.5, color=POS_FILL, alpha=0.4)
            ax2.fill_between(sig["date"], 0.5, sig["roll_acc"],
                             where=sig["roll_acc"] < 0.5, color=NEG_FILL, alpha=0.4)
