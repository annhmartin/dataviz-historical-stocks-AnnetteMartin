...
st.markdown("Shows 30 days before and 15 days after a major price move where sentiment preceded it. Orange line = big move. Purple dashed = sentiment signal.")

valid = [t for t in selected if t in df_key_moves["ticker"].unique()]
if not valid: st.info("No key moves found for tickers in selected sector."); st.stop()
...
if km.empty: st.info("No key moves found. Try unchecking Correctly Predicted Only or expanding the date range."); st.stop()
...
if not price_w.empty: price_w = price_w[(price_w["Date"]>=win_start) & (price_w["Date"]<=win_end)]
if price_w.empty: st.warning("No price data for "+ticker_km); st.stop()
...
ax1.legend(loc="upper left"); ax1.set_facecolor("white")
if not sig_w.empty:
...
ax2.xaxis.set_major_formatter(mdates.DateFormatter("%b %Y")); ax2.set_facecolor("white")
