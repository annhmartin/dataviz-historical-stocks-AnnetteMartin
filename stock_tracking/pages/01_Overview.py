...
for ticker in tickers_to_plot:
    ...
    if price.empty: st.warning(ticker + ": no price data"); continue
    ...
    ax1.set_ylabel("Sentiment Score"); ax1.set_facecolor("white")
    ax1.set_title(ticker + " - Sentiment + 7-Day % Price Change | " + str(has_signal) + " signal days")
    ax1.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
    if has_signal >= 5: ax1.legend(loc="upper left")
